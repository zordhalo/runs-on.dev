...
//
// A selector-DKIM form `<selector>._domainkey` is accepted for the same
// reason: DKIM publishes its public key at `<selector>._domainkey.<domain>`
// and the selector is chosen by the mail provider (Zoho: `zmail`, Google:
// `google`), so `_domainkey` alone cannot express the host. The service
// label is fixed, the selector side follows the ordinary name grammar, and
// the full form is still exactly two labels deep.
...
const SUBDOMAIN_DKIM_LABEL = /^[a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?\._domainkey$/;
...
function validateSubdomains(subdomains, rootName, errors) {
  ...
  for (const label of labels) {
    // Either the one-label form (`_vercel`, `blog`), the two-label
    // verification form (`_vercel.recruitment`), or the DKIM selector form
    // (`zmail._domainkey`). Anything else fails.
    if (!SUBDOMAIN_LABEL.test(label) && !SUBDOMAIN_NESTED_LABEL.test(label) && !SUBDOMAIN_DKIM_LABEL.test(label)) {
...
export function validateRecord(obj) {
  ...
  const recordsOk = validateRecordsShape(obj.records, errors, { allowUrl: true, host: `${obj.name}.runs-on.dev` });
